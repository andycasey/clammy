...
$\mathbf{A}$         & $n\times D$ & full design matrix $[\,\mathbf{T}\mid\mathbf{P}\,]$, $D=(K{+}1)+(L{+}1)$ \\
...
\begin{equation}
  \mathrm{dof} \;=\; n_{\rm good} \;-\; (K+1) \;-\; (L+1) \;-\; n_{\rm nonlin},
\end{equation}
with $n_{\rm good}$ the number of unmasked pixels, $(K+1)$ template weights,
$(L+1)$ continuum coefficients, and $n_{\rm nonlin}\in\{1,2\}$ the number of
fitted nonlinear parameters ($p$ alone, or $p$ and $\sigma_{\rm pix}$).
